#' Add zigzag legs indicator
#'
#' version for oldstyle quantmod:::charSeries
#'
#' @keywords internal
#' @export
`addZigZagLegVo` <- function(legvolume, legside, log.scale=FALSE ) {
   lchob <- quantmod:::get.current.chob()
   x <- as.matrix(lchob@xdata)

  legside <- as.matrix(legside)
  Volumes <- as.matrix(legvolume)
...
#    multi.col <- x@params$multi.col
    ...
    legend.text <- list(list(
           legend=c(paste("Cum Volume (",vol.scale[[2]],"):",sep=''),format(last(Volumes)*vol.scale[[1]],big.mark=',')),
